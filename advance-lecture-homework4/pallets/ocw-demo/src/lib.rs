...
impl<T: Trait> Module<T> {
	...
	fn fetch_dot_price() -> Result<(), Error<T>> {
		...
		if let Some(Some(dot_price)) = s_info.get::<u32>() {
		...
		if let Ok(_guard) = lock.try_lock() {
			match Self::fetch_n_parse() {
				Ok(price) => { 
					s_info.set( &price); 
					let call = Call::submit_number_unsigned(price);
					SubmitTransaction::<T, Call<T>>::submit_unsigned_transaction(call.into())
					.map_err(|_| {
						debug::error!("Failed in offchain_unsigned_tx");
						return <Error<T>>::OffchainUnsignedTxError;
					});
				}
	...
	fn fetch_n_parse() -> Result<u32, Error<T>> {
		...
		let price_str = pr_info["data"]["priceUsd"].as_str().unwrap_or("");
		let price = (price_str.parse::<f64>().unwrap_or(0.0) * 1000.0) as u32;
		debug::info!("{}", price);
		Ok(price)
	}
